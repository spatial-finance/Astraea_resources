...
def analyze_images(images_path, n_pca_components=20):
...
def run_tsne(images_path, output_path, tsne_dimensions, tsne_perplexity, tsne_learning_rate, n_runs=20):
    np.random.seed(0)
    ...
    for i in range(n_runs):
        tsne = TSNE(n_components=tsne_dimensions, learning_rate=tsne_learning_rate, perplexity=tsne_perplexity, verbose=2, n_iter=10**4, random_state=i, method='exact')
        Y.append(tsne.fit_transform(X))
        tsnes.append(tsne)

    I = np.argmin([tsne.kl_divergence_ for tsne in tsnes])
    tsne = tsnes[I]
    Y = Y[I]
    ...
    for i,f in enumerate(images):
        point = [float((Y[i,k] - np.min(Y[:,k]))/(np.max(Y[:,k]) - np.min(Y[:,k]))) for k in range(tsne_dimensions) ]
        data.append({"path":os.path.abspath(join(images_path,images[i])), "point":point})
    with open(output_path, 'w') as outfile:
        json.dump({'data': data, 'kl_divergence': tsne.kl_divergence_}, outfile)
